  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!supabase) { console.error("[komerce-webhook] supabase not configured."); return ok(); }
        ...
        if (!orderIdFromKomerce) { console.error("[komerce-webhook] no order_id in body"); return ok(); }
        const orderSerial = orderIdFromKomerce.replace(/^INV-/, "");

        const { data: order, error: findError } = await supabase
          .from("store_orders").select("*").eq("serial", orderSerial).maybeSingle();
        if (findError || !order) { console.error("[komerce-webhook] order not found for serial:", orderSerial); return ok(); }
        ...
        if (!receivedSignature || computedSignature !== receivedSignature) {
          console.error("[komerce-webhook] signature mismatch, request ignored:", orderSerial);
          return ok();
        }
        ...
        try {
          ...
          if (error) { console.error("[komerce-webhook] failed to update order:", error.message); return ok(); }
          ...
          if (isPaid && order.status === "pending") {
            ...
            if (voucherError) console.error("[komerce-webhook] order marked paid, but failed to issue voucher:", voucherError.message);
