...
import { Trash2, Save, Sparkles, Plus, Check, X } from "lucide-react";
import {
...
export type Access = { pw: string; code?: undefined } | { pw?: undefined; code: string };
...
export function CalendarPanel({ access, classes, canEdit }: { access: Access; classes: ClassOpt[]; canEdit: boolean }) {
  ...
  async function add() {
    ...
    const r = await saveCalendarEvent({ data: { password: access.pw, classId: classId || undefined, title, description: desc, eventDate: date, eventType: type } });
    setBusy(false);
  ...
  async function remove(id: string) {
    if (!access.pw) return;
    await deleteCalendarEvent({ data: { password: access.pw, id } });
    setReload((x) => x + 1);
  }
