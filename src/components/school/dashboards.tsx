...
function StaffProfileForm({ pw, staffId, onSaved }: { pw: string; staffId: string; onSaved: () => void }) {
  ...
  async function save() {
    ...
    const r = await saveMyStaffProfile({ data: { password: pw, staffId, ...f, fullName: f.fullName ?? "" } });
    setBusy(false);
...
function StudentProfileForm({ code, onSaved }: { code: string; onSaved: () => void }) {
  ...
  async function save() {
    if (!f.fullName?.trim()) { setErr("Nama lengkap wajib diisi."); return; }
    setBusy(true); setErr(null);
    const r = await saveMyStudentProfile({ data: { code, ...f, fullName: f.fullName ?? "" } });
    setBusy(false);
    if (!r.ok) { setErr(r.error); return; }
    onSaved();
  }
      ...
      <div>
        ...
        {view === "student" && (
          <div>
            ...
            {academicSub === "activity" && <AllActivitiesView division={null} />}
            {academicSub === "assessment" && <AssessmentResultsPanel pw={pw} />}
...
export function HosDashboard({ staffId }: { staffId: string }) {
  ...
  const tabs = [
    ...
    ...calendarOnlyTab, { id: "gallery", label: "Gallery" }, { id: "settings", label: "Settings" },
  ];
  return (
    <div>
...
export function AdminHosDashboard() {
  ...
  const tabs = [
    ...
    { id: "announce", label: "Announcements" }, ...ACADEMIC_TABS.filter((t) => t.id !== "assessment"), PIN_TAB,
  ];
  return (
    <div>
...
export function TeacherDashboard({ staffId, staffName, role, defaultClassId, division }: { staffId: string; staffName: string; role: string | null; defaultClassId: string | null; division?: string | null }) {
  ...
  const tabs = [
    ...
    { id: "lesson", label: "Lesson Plan" }, { id: "projects", label: "Official Letter" },
    { id: "childassessment", label: "Child Assessment" },
...
const PARENT_MENU = [
  ...
  { id: "gallery", label: "Gallery", Icon: BookOpen, color: "bg-indigo-500/15 text-indigo-600" },
];
  ...
  if (section) {
    ...
    return (
      <div className="max-w-lg mx-auto">
        ...
        {section === "assessment" && <ParentAssessmentReportsPanel code={code} />}
        ...
        {section === "gallery" && <GalleryPanel code={code} />}
      </div>
    );
  }
